...
class App:
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="📦 骰子模拟器", font=("Arial", 18, "bold"),
        ...
        # 根据工具名称添加特定功能
        if "timer" in "dice-roller".lower() or "clock" in "dice-roller".lower():
            self.setup_timer_ui(main)
        elif "random" in "dice-roller".lower() or "dice" in "dice-roller".lower() or "coin" in "dice-roller".lower():
            self.setup_random_ui(main)
        elif "note" in "dice-roller".lower() or "todo" in "dice-roller".lower():
    ...
    def generate_random(self):
        if "dice" in "dice-roller".lower():
            result = random.randint(1, 6)
            self.result_label.config(text=f"🎲 {result}")
        elif "coin" in "dice-roller".lower():
